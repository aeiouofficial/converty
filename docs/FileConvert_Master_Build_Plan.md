# FileConvert Universal Windows Right-Click Converter
...
```text
Explorer.exe
  └─ FileConvert.ShellExtension.dll (native C++; tiny, no media parsing)
       └─ launches FileConvert.Bridge.exe (signed standalone executable)
            └─ authenticated/bounded named-pipe request
                 └─ FileConvert.Host.exe (.NET 10 coordinator; no media parsers)
                      ├─ queue/journal/policy/settings/capability registry
                      ├─ private staging manager
                      └─ launches disposable restricted workers
                           ├─ FileConvert.ProbeWorker.exe
                           └─ FileConvert.EngineWorker.exe
...
Microsoft notes that a default named-pipe security descriptor grants read access to Everyone and the anonymous account [MS-2]. FileConvert therefore creates explicit security descriptors.
...
### Recovery journal
Persist only the minimum job state needed to identify abandoned private temp files after Host crash/reboot. On startup, clean only paths tied to valid FileConvert job metadata beneath FileConvert-owned staging roots.
...
│  ├─ FileConvert.Core/
│  ├─ FileConvert.Ipc/
│  ├─ FileConvert.Security/
│  ├─ FileConvert.Host/
│  ├─ FileConvert.Bridge/
│  ├─ FileConvert.ProbeWorker/
│  ├─ FileConvert.EngineWorker/
│  ├─ FileConvert.Settings/
│  └─ FileConvert.Serialization/
├─ native/
│  └─ FileConvert.ShellExtension/
├─ providers/
│  ├─ FileConvert.Provider.FFmpeg/
│  └─ FileConvert.Provider.Wic/
├─ schemas/
├─ packaging/
│  └─ FileConvert.Package/
├─ tests/
