...
Current workspace version: `0.1.0-dev.14`. Repository `main` is the only durable completed authority. Re-fetch live `main` before every write and completion claim.
...
Expand the fixed typed Audio conversion preset/action matrix through the existing Strict Worker/provider path, test-first, without raw FFmpeg passthrough, PATH lookup, network dependency or containment weakening.
