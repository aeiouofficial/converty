...
public static class TrustedFfprobePath
{
    public const string ExecutableFileName = "ffprobe.exe";
    ...
    public static string Resolve(string applicationDirectory)
    {
        ...
        if (!Directory.Exists(engineDirectory))
        {
            throw new FileNotFoundException(
                "Bundled Converty FFprobe directory is missing.",
                Path.Combine(engineDirectory, ExecutableFileName));
        }

        RejectReparsePoint(engineDirectory, "Bundled Converty FFprobe directory");

        string executablePath = Path.GetFullPath(Path.Combine(engineDirectory, ExecutableFileName));
        if (!File.Exists(executablePath))
        {
            throw new FileNotFoundException("Bundled Converty ffprobe.exe is missing.", executablePath);
        }

        RejectReparsePoint(executablePath, "Bundled Converty ffprobe.exe");
