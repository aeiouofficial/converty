...
try {
    ...
    if ($actual -ne $expected) {
        throw "Converty requires .NET SDK $expected; active SDK is $actual."
    }
    ...
    if ($GenerateLockFiles) {
        dotnet restore Converty.slnx --use-lock-file --force-evaluate
        ...
        dotnet restore Converty.slnx --locked-mode
